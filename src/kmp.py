...
class GMM:
    def __init__(self, num_vars, data, num_states=8, dt=0.003):
...
class KMP:  # Assumptions: Input is only time; All dofs of output are continuous TODO: Generalize
    def __init__(self, input_dofs, robot_dofs, demo_dt, ndemos, data_address, output_dt):
        ...
        self.gmm_model = GMM(num_vars=(self.input_dofs + self.robot_dofs), data=self.data, dt=output_dt)
...
if __name__ == "__main__":
    data_addr = '../training_data/KMP_4pts_largerboundaries/'
    # ndemos = len(os.listdir(data_addr))
    ndemos = 30
    kmp = KMP(input_dofs=1, robot_dofs=4, demo_dt=0.01, ndemos=ndemos, data_address=data_addr, output_dt=0.003)

    # Set KMP params (This dt is KMP's dt)
    kmp.setParams(lamda=0.001, kh=10)
    filehandler = open("../pickled_objects/kmp_lamda0.001_kh6_150.obj", 'wb')
